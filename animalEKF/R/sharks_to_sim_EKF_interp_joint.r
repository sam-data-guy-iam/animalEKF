sharks_to_sim_EKF_interp_joint <- function(env_obj) {
	...
	#print("tis")
	#print(env_obj$Xpart_history[i-1,"time_in_state",, env_obj$s])	
	#print("old state")
	#print(env_obj$Xpart_history[i-1,"lambda",, env_obj$s])					
	#print("new state")
	#print(env_obj$Xpart_history[env_obj$i, "lambda",, env_obj$s])				
	#if the previous interval had an observation for this one
	#because we resample based on it and update to mk from mk_prev
	...
	env_obj$sigma_draw[,, env_obj$s][ cbind(1:env_obj$npart, z) ] <- pmax(1e-15, MCMCpack::rinvgamma(n=env_obj$npart, shape=env_obj$sigma_pars[,, env_obj$s][ cbind(1:env_obj$npart, 2*z -1)], scale=env_obj$sigma_pars[,, env_obj$s][ cbind(1:env_obj$npart, 2*z)]))
	env_obj$tau_draw[,, env_obj$s][ cbind(1:env_obj$npart, z) ]   <- pmax(1e-15, MCMCpack::rinvgamma(n=env_obj$npart, shape=env_obj$tau_pars[,, env_obj$s][ cbind(1:env_obj$npart, 2*z -1)], scale=env_obj$tau_pars[,, env_obj$s][ cbind(1:env_obj$npart, 2*z)]))
	...
	for (p in 1:env_obj$npart) { 
		...
		env_obj$Qt[1:2, 1:2,z,p, env_obj$s] <- MCMCpack::riwish(v=env_obj$Particle_errvar[[ env_obj$s ]][[ p ]]$dof, S=env_obj$Particle_errvar[[ env_obj$s ]][[ p ]]$sig)
		...
		env_obj$mk_prev[,z,p, env_obj$s] <- env_obj$f(mk=env_obj$mk_actual[,p, env_obj$s], new_logv=env_obj$logv_angle_mu_draw[p,"logv",z, env_obj$s], 
											 theta=normalize_angle(env_obj$logv_angle_mu_draw[p,"turn",z, env_obj$s]), dtprev=env_obj$reg_dt) #a_{t+1}
		
		Fx_tmp <- env_obj$Fx(mk=env_obj$mk_actual[,p, env_obj$s], dtprev=env_obj$reg_dt)

		env_obj$Pk_prev[,,z,p, env_obj$s] <- as.matrix(Matrix::nearPD(Fx_tmp %*% env_obj$Pk_actual[,,p, env_obj$s] %*% t(Fx_tmp) + env_obj$Qt[,,z,p, env_obj$s], ensureSymmetry=TRUE)$mat ) #R_{t+1}

		#FxP <- Fx_tmp %*% env_obj$Pk_actual[,,p, env_obj$s]
		
		#env_obj$Pk_prev[,,z,p, env_obj$s] <- as.matrix(Matrix::nearPD(env_obj$Pk_actual[,,p, env_obj$s] + FxP + t(FxP) + (FxP %*% t(Fx_tmp)) + env_obj$Qt[,,z,p, env_obj$s], ensureSymmetry=TRUE)$mat ) #R_{t+1}

		
		if (env_obj$truncate_to_map) {	
			tmp <- reject_sampling(mu=env_obj$mk_prev[,z,p, env_obj$s], cmat=env_obj$Pk_prev[,,z,p, env_obj$s], prev_val=env_obj$Xpart_history[env_obj$i-1 , c("X","Y","logv","bearing_rad"), p, env_obj$s], obj=env_obj)
			env_obj$Xpart_history[env_obj$i , c("X","Y","logv","bearing_rad"), p, env_obj$s] <- tmp$val
			env_obj$num_reject[ p ] <- tmp$iter
		}
		else {
			env_obj$Xpart_history[env_obj$i, c("X","Y","logv","bearing_rad"), p, env_obj$s] <- mvtnorm::rmvnorm(n=1, mean=env_obj$mk_prev[,z,p, env_obj$s], sigma=env_obj$Pk_prev[,,z,p, env_obj$s])
		
		}	
		#env_obj$Xpart_history[i , c("X","Y","logv","bearing_rad"), p, env_obj$s] <- reject_sampling(mu=f(mk=env_obj$Xpart_history[i-1 , c("X","Y","logv","bearing_rad"), p, env_obj$s], theta=logv_angle_draw[p,"turn",z], dtprev=reg_dt), cmat=Qt, prev_val=env_obj$Xpart_history[i-1 , c("X","Y","logv","bearing_rad"), p, env_obj$s])$val					
   
   
		
		#now store in mk_actual so update for next time.
		#mk_actual[,p, env_obj$s] <- mk_prev[,z,p, env_obj$s]
		#mk_actual later takes the mk vector that is drawn, but need to store Pk_actual
