	...
	if (env_obj$nstates > 1) {
		...
		if (env_obj$time_dep_trans==FALSE) {
			...
			for (ss in env_obj$shark_names) {
				...
				plot(p2to1_density_byregion[[ ss ]][[ 1 ]], ylab="density", xlim=c(0,1), ylim=c(0,ylim_trans),col=1, lty=2, main=paste("Density of p(2->1) by region, shark", ss), xlab="Probability", las=1)
				...
				if (env_obj$smooth_parameters) {
					plot(p2to1_density_byregion_smoothed[[ ss ]][[ 1 ]], ylab="density", xlim=c(0,1), ylim=c(0,ylim_trans),col=1, lty=2, main=paste("Density of p(foraging) by region, shark",ss, "(smoothed)"), xlab="Probability", las=1)
