	...
	for (s in env_obj$sharks_with_obs) {
		...
		for (p in 1:env_obj$npart) {
			...
			for (y in 1:env_obj$yobs_sharks[ s ]) {	
				...
				env_obj$Pk[,,z,p,s] <- as.matrix(Matrix::nearPD(env_obj$Pk[,,z,p,s] + env_obj$Pk_prev[,,z,p,s] - ktmp %*% env_obj$SigY[[ s ]][y,z,p] %*% t(ktmp), ensureSymmetry=TRUE)$mat)	
					
				env_obj$mk[,z,p,s] <- env_obj$mk[,z,p,s] + (ktmp %*% t(env_obj$ynext[ which(rownames(env_obj$ynext)==s)[ y ], "X", drop=FALSE] - env_obj$h(mk=mk_tmp, dtprev=j_tmp[ y ] * env_obj$reg_dt)))
