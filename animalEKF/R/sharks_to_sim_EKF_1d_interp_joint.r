sharks_to_sim_EKF_1d_interp_joint <- function(env_obj) {
	...
	env_obj$sigma_draw[access_sigma] <- pmax(1e-15, MCMCpack::rinvgamma(n=env_obj$npart, shape=env_obj$sigma_pars[,,env_obj$s][ cbind(1:env_obj$npart, 2*z -1)], scale=env_obj$sigma_pars[,,env_obj$s][ cbind(1:env_obj$npart, 2*z)]))
	...
	for (p in 1:env_obj$npart) { 
		...
		env_obj$Qt[,,z,p,env_obj$s]  <- diag(c(MCMCpack::riwish(v=env_obj$Particle_errvar[[ env_obj$s ]][[ p ]]$dof, S=env_obj$Particle_errvar[[ env_obj$s ]][[ p ]]$sig), env_obj$sigma_draw[p, z, env_obj$s]))
		...
		env_obj$mk_prev[,z,p,env_obj$s]  <- env_obj$f(mk=env_obj$mk_actual[,p,env_obj$s] , new_logv=env_obj$logv_angle_mu_draw[p,"logv",z, env_obj$s], dtprev=env_obj$reg_dt) #a_{t+1}
		...
		env_obj$Xpart_history[env_obj$i , c("X","logv"), p, env_obj$s]  <- mvtnorm::rmvnorm(n=1, mean=env_obj$mk_prev[,z,p,env_obj$s] , sigma=env_obj$Pk_prev[,,z,p,env_obj$s] )
